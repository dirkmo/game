...
const uint8_t console_font_6x8[] = {
...
int main(int argc, char*  argv[]) {
    FILE *f = fopen("font_6x8.c", "w");
    fprintf(f, "const uint8_t font[] = {\n");
    for( int i = 0; i < 256; i++ ) {
        fprintf(f, "  // 0x%02X ('%c')\n", i, iscntrl(i) ? 0 : i);
        const uint8_t *c = &console_font_6x8[(uint8_t)i * 8];
        for( int b = 7; b >= 2; b-- ) {
            uint8_t val = 
                (((c[0] >> b)&1) << 0)
              | (((c[1] >> b)&1) << 1)
            ...
            for( int valb = 7; valb >= 0; valb-- ) {
                fprintf(f, "%d", (val >> valb)&1);
            }
            fprintf(f, "\n");
        }
        fprintf(f, "\n");
